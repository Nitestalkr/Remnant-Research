...
const CONVERGENCE  = 0.3; // 1 - DAMPING (0.0 = no convergence, 1.0 = full cluster adoption)
...
// Legacy compatibility: DAMPING is still supported (DAMPING = 1 - CONVERGENCE)
...
function main() {
  ...
  // 4. Apply convergence: new = prev * (1-CF) + cluster_avg * CF
  const newWeights = {};
  const floorsApplied = [];
  for (const drive of DRIVES) {
    const prev = prevWeights[drive] ?? clusterAvg[drive];
    const raw  = r3(prev * (1 - CONVERGENCE) + clusterAvg[drive] * CONVERGENCE);
  ...
  const config = {
    ...
    syncedAt:        timestamp,
    convergence:     CONVERGENCE, // 1 - DAMPING (0.0 = no convergence, 1.0 = full cluster adoption)
    ...
    note: 'Scale flipped: convergence_factor (0.0→1.0) replaces damping (1.0→0.0). Higher = more convergence.'
  };
